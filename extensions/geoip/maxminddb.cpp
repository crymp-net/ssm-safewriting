...
LOCAL int map_file(MMDB_s *const mmdb)
{
    ...
    HANDLE fd = CreateFileW(utf16_filename, GENERIC_READ, FILE_SHARE_READ, NULL,
...
LOCAL int resolve_any_address(const char *ipstr, struct addrinfo **addresses)
{
	...
	hints.ai_socktype = SOCK_STREAM;  // We set ai_socktype so that we only get one result back
